...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, metric_method, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        prediction = []  # Store all batch outputs
        ...
        # Print the shapes of input, prediction, and data target tensor
        print('input:', input.shape)
        print('prediction:', prediction.shape)
        print('data_target_tensor:', data_target_tensor.shape)

        # Print the last predicted value
        print('Last predicted value:', prediction[-1, :, :])
        # Save the last predicted value to an NPZ file
        np.savez(os.path.join(params_path, f'last_predicted_value_epoch_{global_step}.npz'), last_predicted_value=prediction[-1])
        ...
        metrics_file = os.path.join(params_path, 'final_metrics.txt')
